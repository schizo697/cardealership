                    ...
                    <form action="{{ route('services.update', $services->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                {{-- For new customer --}}
                                <div id="newUserDiv">
                                    <input type="text" name="customer_name" class="form-control" value="{{ $services->user_id ? $services->user->name : $services->customer_name }}">
                                </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="car" id="car" class="form-control" value="{{ $services->car }}">
                                @error('car')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="service" id="service" class="form-control" value="{{ $services->service }}">
                                @error('service')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="selectedMechanics">Mechanics on Work</label>
                                @php
                                    $mechanicIds = explode(',', $services->assigned_mechanic);
                                    $mechanics = \App\Models\User::whereIn('id', $mechanicIds)->get();
                                @endphp
                            
                                <div id="selectedMechanics" style="margin-top: 10px;">
                                    @foreach ($mechanics as $index => $mechanic)
                                        <li>{{ $mechanic->name }}</li>
                                    @endforeach
                                </div>
                            
                                <input type="hidden" name="mechanic_ids" id="mechanic_ids" value="{{ implode(',', $mechanics->pluck('id')->toArray()) }}">
                            </div>                            
                        </div>
